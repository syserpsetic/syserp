...
@section('subcontent')
                ...
                <div class="flex flex-1 items-center justify-center px-5 lg:justify-start">
                    ...
                    <div class="ml-5">
                        <div class="w-240 truncate text-lg font-medium sm:w-80 sm:whitespace-normal">
                            ...
                            <h1 class="text-5xl font-medium leading-none">CATEGORÍAS</h1>
                        </div>
                        <div class="text-slate-500">Pantalla de administración de Categorías.</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="intro-y box mt-5 p-5">
                ...
                <div class="p-5">
                    <h3 class="text-2xl font-medium leading-none"><div class="flex items-center">
                        <i data-lucide="List" class="w-6 h-6 mr-1"></i>
                            <span class="text-white-700"> Lista de Categorías</span>
                        </div></h3>
                </div>
            </div>
            <div class="intro-y col-span-6 lg:col-span-6 text-right">
                <div class="p-5">
                    @if(in_array('zeta_escribir_categorias', $scopes))
                        <x-base.button
                            class="mb-2 mr-1"
                            variant="primary"
                            id="btn_nueva_categoria"
        ...
        <!-- BEGIN: Modal Content -->
        <x-base.dialog id="modal_nueva_categoria">
            <x-base.dialog.panel>
                <x-base.dialog.title class="bg-primary">
                    <h2 class="mr-auto text-white font-medium">
                        <div class="flex items-center">
                        <i data-lucide="Plus" class="w-4 h-4 mr-1"></i>
                            <span class="text-white-700"> Nueva Categoría</span>
                        </div>
                    </h2>
                </x-base.dialog.title>
                <x-base.dialog.description class="grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12 sm:col-span-12">
                        <x-base.form-label class="font-extrabold" for="modal_input_nombre_categoria">
                            Nombre
                        </x-base.form-label>
                        <x-base.form-input id="modal_input_nombre_categoria" type="text" placeholder="Nombre de la categoría" />
                    </div>
                    <div class="col-span-12 sm:col-span-12">
                        <x-base.form-label class="font-extrabold" for="modal_input_descripcion_categoria">
                            Descripción
                        </x-base.form-label>
                        <x-base.form-input id="modal_input_descripcion_categoria" type="text" placeholder="Describa la categoría" />
                    </div>
                ...
                <x-base.dialog.footer class="bg-dark">
                    ...
                    <x-base.button size="sm" class="w-20" type="button" variant="primary" id="modal_btn_guardar_categoria">
                        Guardar
                    </x-base.button>
            ...
            <x-base.dialog.panel>
                <div class="p-5 text-center">
                    ...
                    <div class="mt-2 text-slate-500">
                        ¿Realmente desea eliminar esta Categoria?<br />
...
@once
    @push('scripts')
        ...
        <script type="module">
            ...
            var tabulator = null;
            var url_categorias_data = "{{url('/configuracion/categorias/data')}}";
            var url_guardar_categorias = "{{url('/configuracion/categorias/guardar')}}";
            var titleMsg = null;
            var textMsg = null;
            var typeMsg = null;
            var numerofila = null;
            ...
            $(document).ready(function() {
                ...
                $('#sdatatable tbody').on('click', '.editar', function() {
                    ...
                    id = data[0];
                    $("#modal_input_nombre_categoria").val(data[1]);
                    $("#modal_input_descripcion_categoria").val(data[2]);
                    const el = document.querySelector("#modal_nueva_categoria");
            ...
            $("#btn_nueva_categoria").on("click", function (event) {
                $("#modal_input_nombre_categoria").val('');
                $("#modal_input_descripcion_categoria").val('');
                accion = 1;
                const el = document.querySelector("#modal_nueva_categoria");
                const modal = tailwind.Modal.getOrCreateInstance(el);
                modal.show();
            ...
            $("#modal_btn_guardar_categoria").on("click", function () {
                nombre = $("#modal_input_nombre_categoria").val();
                descripcion = $("#modal_input_descripcion_categoria").val();
